...
public class DynamicListTestView : OrbitView {
    //By using ObservableCollection instead of List, we don't need to worry about refreshing the list at all, it will happen automatically when the content changes
    [ValueID] private ObservableCollection<string> strings = new();
    [ValueID] private string enteredText = "";

    [ListenFor("RemoveString")]
    private void RemoveString(string str) => strings.Remove(str);
